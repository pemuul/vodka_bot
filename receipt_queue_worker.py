...
4. Убедитесь, что зависимости установлены (``pip install -r requirements.txt``).
   ...
   ``tesseract-ocr`` (используется как fallback при сбое EasyOCR). Языки и
   сеть распознавания EasyOCR настраиваются переменными окружения
   ``OCR_LANGUAGES`` (по умолчанию только ``ru``) и ``OCR_RECOG_NETWORK``
   (по умолчанию ``cyrillic_g2``). Если нужно явно выгружать модели EasyOCR
   после каждого чека, установите ``OCR_FORCE_RELEASE=1``; по умолчанию воркер
   держит прогретый процесс, чтобы не загружать веса при каждом запросе.
