...
14. Обновление статуса чека и **обязательное** уведомление пользователя. Текст берётся из
    единого списка `receipt_validation.STATUS_USER_MESSAGES` — по одному на каждый статус из
    `FINAL_RECEIPT_STATUSES`, включая `"На ручной проверке"` и `"Ошибка"` (раньше оба
    молчали, и чек повисал без ответа до тех пор, пока администратор случайно не заметит его
    в списке — на проде до 17 часов). Для накопительного прогресса этапа (оба типа) вместо
    статусного текста уходит сообщение этапа (см. "Гарантированный приз" ниже). Статус, у
    которого не нашлось текста, пишется в лог как ошибка; тест
    `test_receipt_notifications.py::TestStatusMessages` не даёт добавить статус без текста.
