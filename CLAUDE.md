...
- `site_bot/main.py: save_draw()` блокирует (`400`) сохранение, если в одном payload
  ...
  запись.
- **`save_draw()` апсертит этапы/правила по `id`, не удаляет-и-пересоздаёт** (исправлено
  2026-08-31, живой баг на проде). Раньше КАЖДОЕ сохранение формы `/prize-draws` — даже
  правка одного текстового поля — удаляло ВСЕ этапы и правила акции и вставляло их заново с
  новыми id. Поскольку `receipts.stage_id` и `receipt_items.matched_rule_id` — постоянные
  ссылки (накопительный прогресс, «пиновка» чека к этапу, см. выше и раздел "Гарантированный
  приз"), это молча обнуляло прогресс всех участников этапа при любом ресейве — конкретно
  поймано на проде: пользователь накопил 1 из 2 нужных товаров, админ пересохранил акцию
  (id правила сменился), второй чек засчитался уже под НОВЫМ id и не видел старый прогресс —
  гарантированный приз никогда не выдавался. Теперь `StageIn`/`RuleIn` несут свой реальный
  `id` (фронтенд уже это делал, бэкенд просто игнорировал поле), и `save_draw()` для каждого
  этапа/правила из payload делает `UPDATE` по существующему id (если он принадлежит этой
  акции/этапу) или `INSERT`, если `id is None`; после прохода все id этапов, отсутствующие в
  payload, удаляются вместе с их правилами/победителями — так админ по-прежнему может удалить
  этап/правило кнопкой в UI. **Победителей (`prize_draw_winners`) форма теперь вообще не
  трогает** для сохраняемых этапов (раньше тоже удалялись-и-вставлялись из payload клиента —
  тот же класс бага: если бот записывал победителя, пока у админа было открыто окно
  редактирования, следующий "Сохранить" мог его стереть); победители пишутся напрямую в БД
  через `determine()`/`evaluate_guaranteed_prize_stage()`, эта форма их только отображает.
  Регрессия — `tests/test_site_bot_progress.py::TestSaveDrawPreservesIds`.
...
- `test_site_bot_progress.py` — веб-часть (`site_bot/main.py`) покрыта минимально: новые
  ...
  баг с лишними лотерейными билетами, см. "Гарантированный приз" выше. Аналогично
  `TestSaveDrawPreservesIds` вызывает `save_draw()` напрямую (тоже без `Depends()`) — регрессия
  на баг с обнулением id этапов/правил при пересохранении формы, см. выше.
