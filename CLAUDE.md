...
| **receipt_worker** | `run_receipt_worker.py` → `receipt_queue_worker.main()` | OCR + ФНС для очереди чеков; раз в 5 минут двигает этапы по датам (`sql_mgt.activate_due_stages()`) |
| **site** | `site_bot/site_flusk_run.py` | FastAPI веб-интерфейс |

Шаблон конфига supervisor — `new_bot_file/supervisor.conf`.

**Общее для бота и воркера:** оба навешивают на свой `Bot` middleware
`outgoing_logger.RequestLogger`, который пишет исходящие в `participant_messages`. Раньше он
жил внутри `bot.py` и подключался только в процессе бота — поэтому ответы воркера («✅ Чек
подтверждён», «❌ В чеке не найден нужный товар») уходили пользователю, но не попадали в
историю переписки, и в админ-панели чат выглядел так, будто бот на чеки не отвечает вовсе.
Запись делается ПОСЛЕ успешной отправки и сохраняет `tg_message_id`; логируется и `caption`,
а не только `text` (иначе каталог с подписью после первого чека не попадал в историю).

**Смена этапов по датам** живёт в воркере, потому что это единственный постоянно работающий
фоновый процесс: cron в проекте не используется, а бот перезапускается по расписанию.
`get_active_stage()` требует одновременно `status='active'` и попадание даты в диапазон
этапа, и раньше статус не менял никто — акция просто останавливалась молча (04–09.09.2026 в
базе нет ни одного чека именно поэтому). `activate_due_stages()` идемпотентна и сохраняет
инвариант «активен не более одного этапа во всей БД»: следующий включается, только если
после завершения истёкших не осталось ни одного активного.
...
| participant_messages | Полный лог диалога бота с пользователем. `tg_message_id` — идентификатор сообщения в Telegram (без него отправленное сообщение уже не изменить и не удалить; Telegram разрешает боту удалять свои сообщения только 48 часов), `receipt_id` — необязательная ссылка на чек, по которому отправлен ответ (`sql_mgt.get_receipt_bot_messages()`). Индекс по `receipt_id` **частичный** (`WHERE receipt_id IS NOT NULL`, создаётся в `sql_mgt.create_indexes()`): ссылка на чек есть у долей процента строк, полный индекс по самой большой таблице базы рос бы вместе со всей перепиской впустую |
...
14. Обновление статуса чека и уведомление пользователя. Текст берётся из единого списка
    `receipt_validation.STATUS_USER_MESSAGES` (раньше таблица дублировалась в боте и в
    панели, с разным набором статусов). **`"На ручной проверке"` и `"Ошибка"` намеренно НЕ
    пишут пользователю ничего** — решение владельца: чек ждёт администратора на `/receipts`,
    и человек получает ответ тогда, когда решение действительно принято, без промежуточного
    «мы посмотрим» и обещания срока. Эти статусы перечислены в
    `receipt_validation.SILENT_RECEIPT_STATUSES`; лекарство от «чек висит без ответа» — не
    сообщение пользователю, а то, что чек виден АДМИНИСТРАТОРУ (см. `/api/notifications`
    ниже). Для накопительного прогресса этапа (оба типа) вместо статусного текста уходит
    сообщение этапа (см. "Гарантированный приз" ниже). Статус, у которого нет ни текста, ни
    пометки «молчим», пишется в лог как ошибка; тест
    `test_receipt_notifications.py::TestStatusMessages` не даёт добавить статус, про который
    забыли.
15. **Лента уведомлений администратора** (`GET /api/notifications`, колокольчик в панели) —
    единственное место, где видно чек, ждущий человека, поэтому она обязана показывать
    `"На ручной проверке"` наравне с `"Ошибка"`. До 2026-09-15 отбор шёл строго по
    `"Ошибка"`, а этот статус с переездом правил на уровень этапа больше не выставляется
    (последний такой чек на проде — 16.06.2026): чеки на ручной проверке не были видны
    нигде, и при намеренном молчании бота (п. 14) человек ждал ответа до 17 часов. Там же
    показываются чеки, застрявшие в `"В авто обработке"` дольше
    `site_bot/main.py: STUCK_PROCESSING_MINUTES` — признак упавшего или отставшего воркера,
    который раньше тоже не был виден ниоткуда.
    Перед отправкой проверяется `receipt_validation.is_contradictory_acceptance()`: если чек
    подтверждён, а прогресс по ВСЕМ правилам этапа нулевой, сообщение о принятом чеке
    подавляется (иначе пользователь получает «Чек принят» вместе с «0 попыток выиграть» —
    реально ушло человеку трижды 14.09.2026) и заменяется нейтральным подтверждением
...
- **Сообщение приходит при обоих исходах — и `"progress"`, и `"complete"`** (2026-09-15).
  Отдельного «финального» текста у `standard` по-прежнему нет — удалённые в 2026-08-31
  `build_standard_qualify_message()` / `DEFAULT_STANDARD_QUALIFY_MESSAGE` не возвращались, —
  но `build_standard_progress_message()` вызывается в обоих случаях. Причина: при
  `min_quantity == 1` исход `"progress"` недостижим в принципе (первый же засчитанный чек
  сразу закрывает правило), поэтому настроенный в карточке этапа текст не отправлялся
  НИКОГДА, а пользователь получал жёстко зашитое `"✅ Чек подтверждён"` — на проде владелец
  вписал в это поле «Чек принят ✅ / Сейчас у вас {entries_count} 🎟️» и не понимал, почему
  оно не приходит. При `"complete"` остаток пуст, и блок `{remaining_items}` вместе с
  заголовком над ним убирается из текста (`receipt_validation.fill_placeholder()`) — иначе в
  дефолтном шаблоне остаётся висеть «Осталось докупить:» без содержимого. Поля «Финальное
  сообщение» и «Сообщение при лишнем чеке» в `/prize-draws` скрыты для типа «Стандарт»
  (видны только для «Гарантированного приза», `stage-guaranteed-only-block` в
  `prize_draws.html`) и для него больше не сохраняются в БД — `win_message_text` у
  `standard`-этапа ботом не читается.
